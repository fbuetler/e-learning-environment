...
The receiver assume that this was the word the sender wanted to sent. This of course work only if there are not more than 1 error in the word. If this happens the editing distance to another word is closer than to the original word and hence the word is misinterpreted. To counter this, sender and receiver might agree on a bigger minimal distance, but this comes with a trade off. When choosing a bigger minimal distance for a fixed alphabet, the number of words that can be used shrinks. To maintain the same amount of words the size of the alphabet can be increased too, resulting in longer words.
...
\begin{example}
  ...
  \begin{itemize}
    \item \nameref{subsection:addingLetter} - arm: arme, arms, darm, farm, warm
    \item \nameref{subsection:changingLetter} - buch: auch, bach, bush, euch, huch, such, tuch
    \item \nameref{subsection:removingLetters} - bauch: auch, bach, buch
  \end{itemize}
\end{example}
...
The \code{Swap.vue} component reuses some parts of the just mentioned components. It also generates a random amount of each item. The total amount of items is reducible and pupils are asked to represent the same sum over all items but with fewer items. How such a configuration is achieved, is shown in listing \ref{lst:reducableItems}.
